...
R0-R2 are the active roadmap stages. The implemented/tested surface is deliberately narrow:
...
The R2 tests exercise signed identity and lease verification, bounded transitive attenuation, full capability-manifest binding, generation-checked lifecycle transitions, cascading revocation, same-context immutable renewal, live pre-token revalidation, and deterministic authorization race schedules. These tests are evidence for those fixed fixtures and branches only; they are not evidence for the complete T1-T11 target set or for production deployment.
